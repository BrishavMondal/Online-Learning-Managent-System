...
namespace Data_Access.Tables
{
    public class Teachers
    {
        ...
        {
            string query = string.Format("update Teacher set Teacher_Id= '{0}', User_Id= '{1}', Department= '{2}', Contact_Number='{3}' where Teacher_Id= '{0}'", tea.Teacher_Id, tea.User_Id, tea.Department, tea.Contact_Number, tea.Teacher_Id);
            conn.Open();
            SqlCommand cmd = new SqlCommand(query, conn);
