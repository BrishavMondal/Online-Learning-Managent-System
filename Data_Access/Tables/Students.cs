...
namespace Data_Access.Tables
{
    public class Students
    {
        ...
        {
            string query = string.Format("insert into Student ('{2}','{3}','{4}')", stu.Department, stu.Contact_Number, stu.Course);
        ...
        {
            string query = string.Format("update Student set Student_Id= '{0}', User_Id= '{1}', Department= '{2}', Contact_Number= '{3}', Course= '{4}' where Student_Id= '{0}'", stu.Student_Id, stu.User_Id, stu.Department, stu.Contact_Number, stu.Course, stu.Student_Id);
            conn.Open();
            SqlCommand cmd = new SqlCommand(query, conn);
